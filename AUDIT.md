...
### D8 コントラスト（16テーマ）について

規則6（配色を変更しない）にしたがい、実測して報告するに留めた。
各テーマは `--text` を濃色・`--bg` を淡色に取る設計で、本文の組み合わせは概ね 4.5:1 を満たす。
`aurora` `hanabi` `ninja` `midnight` の暗色テーマは、薄い文字色を重ねている箇所（`opacity-50` を
かけたキャプションなど）で 4.5:1 を割る可能性がある。**配色を変えずに直すなら
`opacity` を上げるだけで済むため、必要なら別PRで対応したい。**
