...
| | 32テーマ中の合格 |
|---|---:|
| 白文字 on `--primary` の塗り | 17 |
| 白文字 on `--secondary` の塗り | 9 |
...
| `#14b8a6`（mint） | 2.49:1 | 7.59:1 |
| `#FF6B6B`（きほん） | 2.78:1 | 6.80:1 |

**直しかたは2つしかなく、どちらもアプリの見た目が変わる。**

1. 塗りの色そのものを濃くする → ボタンの色が変わる
2. `--on-primary` / `--on-secondary` を用意し、テーマごとに白か濃い色かを選ぶ
   → きほんテーマの「みんなであそぶ」ボタンの文字が白から濃い色になる

2 のほうが色数は保てるが、いずれにせよ児童が毎日見ている画面の印象が変わる。
規則6（配色を変更しない）に触れるため、**測って報告するに留めている。**
`npm run check:contrast` でいつでも再現できる。

#### 品質ゲートへの組みこみ

`npm run check` に含めた。終了コードに反映するのは、**直しかたが確立しているもの**だけ。

- 本文と補足 → `opacity` を上げれば必ず満たせる
- 強調の文字 → `--primary-d` / `--secondary-d` を使えば満たせる
- `--accent` の上 → `--on-accent` を使えば満たせる

塗りつぶしたボタンの上の文字は、見た目が変わる判断を伴うので報告のみ。CI は止めない。
